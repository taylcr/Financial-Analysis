...
def upload():
    ...
    with open("./urls_content.csv", "rb") as f:
        print(f.name)
        s3.upload_fileobj(f, BUCKET_NAME, "urls_contents.csv", ExtraArgs={"ACL": "public-read"})


    # Upload file to bucket [Show with public-read, and without it too]
    with open("./data.csv", "rb") as f:
        print(f.name)
        s3.upload_fileobj(f, BUCKET_NAME, "data.csv", ExtraArgs={"ACL": "public-read"})
